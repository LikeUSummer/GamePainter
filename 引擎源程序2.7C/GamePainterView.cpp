...
int getTextSize(lua_State* L);  //(str)根据当前字体计算，返回字符串的宽和高
...
int setIMEPos(lua_State* L)
{
	int x=lua_tointeger(L,1);
	int y=lua_tointeger(L,2);
	HIMC hmic = ImmGetContext(g_pWnd->m_hWnd);
	COMPOSITIONFORM cpf;
...
int getTextSize(lua_State* L)
{
	...
	SIZE  size;
	GetTextExtentPoint32(g_BufDC.GetSafeHdc(),str,len,&size);
...
int beginThread(lua_State* L)
{
    ...
    *pID=lua_tointeger(L,1);  //线程编号(0-31)	
	const char* threadName=lua_tostring(L,2);
	if(*pID<0 || *pID>31)
	{
		CString  msg;
        msg.Format("%s线程创建失败 ,Lua脚本传入的线程编号超出了许可范围(0-31)",threadName);
...
{
	int* pID=(int*)pParam;
	lua_pcall(g_stateGroup[*pID],0,0,0);     //用临时状态机调用线程函数
	//退出操作
	DWORD ExitCode=0;
...
void CGamePainterView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if(call)
	{
		...
		g_Box.pushStr(g_word);
		g_Box.pushInt(nRepCnt);
		g_Box.callAnyFunc(2,0); //字符键处理
	}
	/*  //方案二：直接传递原始参数给Lua，当不需要文字处理时，采用下面四行代码即可
	g_Box.loadFunc("OnCKeyDown");
	g_Box.pushStr(nChar);
	g_Box.pushInt(nRepCnt);
	g_Box.callAnyFunc(2,0); //字符键处理
